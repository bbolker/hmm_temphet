...
wbvmsinhmm3s <- fit(sin3, verbose = TRUE, emc=em.control(rand=FALSE, maxit=460))
...
sin5 <- makeDepmix(response = rModels, transition = transition,
                   prior=inMod,homogeneous = FALSE)
wbvmsinhmm5s <- fit(sin5, verbose = TRUE, emc=em.control(rand=FALSE, maxit=460))
...
sin6 <- makeDepmix(response = rModels, transition = transition,
                   prior=inMod,homogeneous = FALSE)
wbvmsinhmm6s <- fit(sin6, verbose = TRUE, emc=em.control(rand=FALSE, maxit=460))
