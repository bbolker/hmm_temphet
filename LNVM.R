...
rModels <- list()
rModels[[1]] <- list()
rModels[[1]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
                                 family = gaussian(), pstart = c(3, 1))
rModels[[1]][[2]] <- vonMises(cat$Turningangle, pstart = c(0, 1),data=cat)

rModels[[2]] <- list()
rModels[[2]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
                                 family = gaussian(), pstart = c(4, 1))
rModels[[2]][[2]] <- vonMises(cat$Turningangle, pstart = c(1, 1),data=cat)

rModels[[3]] <- list()
rModels[[3]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
                                 family = gaussian(), pstart = c(5, 1))
rModels[[3]][[2]] <- vonMises(cat$Turningangle, pstart = c(2, 1))

rModels[[4]] <- list()
rModels[[4]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
...
rModels[[5]] <- list()
rModels[[5]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
                                 family = gaussian(), pstart = c(7, 1))
...
rModels[[6]] <- list()
rModels[[6]][[1]] <- GLMresponse(formula = distance ~ 1, data = cat,
...
mod6 <- makeDepmix(response = rModels, transition = transition,
                   prior=inMod,homogeneous = FALSE)
fm6 <- fit(mod6, verbose = TRUE, emc=em.control(rand=FALSE))
summary(fm6)
