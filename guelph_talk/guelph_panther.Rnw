...
\begin{description}
\item[People] Michael Li, Madelon van de Kerk, Dave Onorato
\item[Support] NSERC Discovery grant
\end{description}
...
\end{itemize}
\item covariates: \\
e.g. habitat map, individual characteristics (sex, age, weight \ldots)
\end{itemize}
...
\begin{columns}
  \begin{column}{0.5\textwidth}
    \begin{itemize}
    \item Florida panther: \emph{Felix ...}
    \item severely reduced habitat, endangered
...
(also {\tt moveHMM} \citepx{michelot_movehmm_2016}, {\tt move.HMM} \citepx{augustine_hmm})
\end{itemize}

\end{frame}

\section[Basic analysis]{Basic analysis (vdK et al.)}
\subsection{}
...
\includegraphics[height=\textheight]{pix/jae_fig1.png}

\end{frame}

\section[Diurnal model]{Incorporating diurnal variation}
\subsection{}
...
\begin{frame}
\frametitle{Temporal patterns}
Figures 3 (and 4?) from paper
\end{frame}
