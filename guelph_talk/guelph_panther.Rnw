...
\end{column}
\begin{column}{0.5\textwidth}
<<arcpic,echo=FALSE,fig.width=3,fig.height=3>>=
x <- c(0,-0.5,0.5,0)
y <- c(0,0.5,1,1.5)
par(mar=c(1,1,1,1),cex=1.5,xpd=NA)
plot(x,y,axes=FALSE,ann=FALSE,pch=16)
eps <- 0.04
arrows(x[-4]+c(-1,1,-1)*eps,y[-4]+eps,x[-1]+c(1,-1,1)*eps,y[-1]-eps,lwd=2)
draw.arc(x[2],y[2],radius=0.2,angle1=-pi/6,angle2=pi/10,col=4)
text(x[2]+0.28,y[2],expression(theta[1]),col=4)
draw.arc(x[3],y[3],radius=0.2,angle1=pi-pi/6,angle2=pi+pi/8,col=4)
text(x[3]-0.28,y[3],expression(theta[2]),col=4)
text((x[2]+x[3])/2,(y[2]+y[3])/2,pos=3,expression(s[1]),col=2)
text((x[3]+x[4])/2,(y[3]+y[4])/2,pos=3,expression(s[2]),col=2)
posvec <- c(4,2,4,3)
for(i in 1:4)
    text(x[i], y[i], substitute(t[j],
                                 list(j = i)),pos=posvec[i])
@ 
\end{column}
\end{columns}
\end{frame}
...
\end{column}
\begin{column}{0.5\textwidth}
<<getdat,echo=FALSE,cache=TRUE,warning=FALSE>>=
load("../allcats.RData")  ## all data
d1 <- subset(dat,cat==1)
g1 <- ggplot(d1,aes(x=LogDist))+
    geom_histogram(binwidth=0.1)+
    labs(x=expression("step length"~(log[10]*m)))
g2 <- ggplot(d1,aes(x=Turningangle))+
    geom_histogram(binwidth=pi/16,alpha=0.2,colour="black",lwd=0.2)+
    coord_polar()+labs(x="",y="")+
    scale_x_continuous(breaks=c(-1,-0.5,0,0.5)*pi,
                       labels=c(180,270,0,90))
print(g1)
@ 

<<g2plot,warning=FALSE>>=
print(g2)
@ 
\end{column}
\end{columns}
...
\includegraphics[width=\textwidth]{pix/distpars.pdf}
...
\includegraphics[width=\textwidth]{pix/parplot.pdf}
...
\begin{frame}
\frametitle{Model complexity: simulation}
<<modsim>>=
...
\begin{column}{0.5\textwidth}
\begin{itemize}
\item Cognition/memory \citepx{bracis_memory_2015}
\item Intraspecific interaction/collective movement \citepx{delgado_statistical_2014}
\item Continuous-time movement models \citepx{calabrese_ctmm_2016}
\item Edges, barriers, and corridors \citepx{beyer_you_2016}
\item Efficient (big-data) approaches \citepx{brillinger_three_2008}
\item Putting it all together ...
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
\includegraphics[width=\textwidth]{pix/kitchensink.jpg}
\end{column}
\end{columns}
\end{frame}
...
\item protocols and tools for model checking \citepx{potts_generalized_2014};
score tests?
...
\begin{frame}
\includegraphics[width=\textwidth]{pix/panther2.jpg}

\url{http://tinyurl.com/panthermoves}; \url{http://www.slideshare.net/bbolker}
\end{frame}
