...
#   + geom_point(aes(shape=factor(nstates),size=1.5,alpha=nstates))
#   + scale_linetype_manual(values=linetypev)
#   + scale_colour_brewer(palette="Dark2")
#   + labs(y=expression(paste(Delta, "BIC"))
#          , x="Number of States") 
...
(ggplot(combdf,aes(x=time,y=steplen,colour=model,lwd=model,alpha=model,linetype=model)) 
  ...
  + scale_linetype_manual(values=linetypev)
) + geom_point(data=vitdf,aes(y=viterbi),lwd=2.5,alpha=1,colour="black",shape=118)
@
...
(ggplot(acfdf,aes(x=lag,y=ACF,colour=model,lwd=model,alpha=model,linetype=model)) 
  + geom_line()
  + xlab("Lag") 
