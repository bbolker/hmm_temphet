...
Model complexity and the number of parameters increase as the number of latent states increase, FMM to HMM, and lastly, FMM and HMM incorporating temporal heterogeneity (Supplementary material, Table 1). The number of free parameters in an HMM can be generalize by summing up the number of free parameters of the three distinct components. Let $n$ be the number of hidden states and $k_{I}, k_{t}, k,{e}$ be the parameters of the prior distribution, transition function and emission distributions. Then the number of free parameters of an HMM is:

K = $$ k_{i} * (n-1) + k_{t}*n*(n-1) + k_{e}*n $$
...
\subsection{Comparing observed data against simulation and Viterbi predictions}
...
df2 <- transform(allmodsumdf2, #change allmodsumdf to allmodsumdf2
            ...
            homog=ifelse(grepl("TH",type),"time-heterogeneous HMM","homogeneous HMM"))
