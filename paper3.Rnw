...
Model complexity and the number of parameters increase as the number of latent states increase, FMM to HMM, and lastly, FMM and HMM incorporating temporal heterogeneity (Supplementary material, Table 1). The number of free parameters in an HMM can be generalize by summing up the number of free parameters of the three distinct components:
