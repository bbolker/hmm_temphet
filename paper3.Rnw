...
little-used extension of the standard HMM framework, 
...
\todo{maybe give the formula for number of parameters (constrained) for each model type here?...ML: Sum(I+T+E), I=Prior=n(s-1) where n is the number of parameters for the prior complexity, T = ns(s-1), E=2s}
...
We used this approach to validate our models and comparing these models with the observed movements instead of the standard Viterbi predictions by the Viterbi algorithm because Viterbi predictions, which use the most probable sequence of movement states based on the observations \citep{zucchini_hidden_2009,langrock_flexible_2012}, double-count the observed data. It is useful to predict missing data in the observation sequence, but because it is conditional on the observed values, it can not reliably evaluate goodness of fit for the different structural complexities of HMM models. \todo{think about/check out \textbf{pseudo-residuals}}

\section{Results}

<<sim_results, warning=FALSE,echo=FALSE,message=FALSE,fig.cap="Simulation test for BIC-optimal approach",fig.width=6,fig.height=4>>=
...
\todo{The description of the simulations should be in Methods, not Results; here we should describe the results of the simulations only}
We simulated a two-state HMM with sinusoidal temporal transitions 100 times and fitted it with two to five state HMMs and without temporal transition.  Figure~\ref{fig:sim_results} shows heterogeneous transition models can always predict the correct number of states, whereas, can overestimate the number of states via BIC-optimal approach. 
...
Figure~\ref{fig:BICred_plot} shows that the BIC-optimal number of states for time homogenous models is consistent with \citeauthor{kerk2015hidden}'s results (Weibull wrapped-Cauchy to Weibull von Mises, and Weibull von Mises to log Normal without turning angles) 
...
sumdat <- transform(readRDS("summary_stats/cat.1.df.RDS")$HMMsumdf,
                    mod = ifelse(grepl("HMM",model),"no","yes")
)
...
## + geom_point(aes(size=parameters))
## + scale_linetype_manual(values=linetypev)
## + scale_colour_brewer(palette="Dark2")
## + scale_shape_discrete(name="# of states")
## + scale_alpha_discrete(name="# of states")    
## + labs(y=expression(paste(Delta, "BIC"))
##        , x="Number of States") 
## + geom_line() 
## + zmargin

### I want this picture but it is too big or something
...
linetypev <- c(rep(c(2,1),c(2,6)),1)
colvec <- c(c(brewer.pal(7,"Dark2"),"black"),"black")
lwdvec <- c(rep(c(1,4),c(7,1)),4)
alphavec <- c(rep(c(1,0.2),c(7,1)),1)
(ggplot(combdf,aes(x=time,y=steplen,colour=model,lwd=model,alpha=model,linetype=model)) 
  + geom_line()
  + xlab("Time of Day") 
  ...
  + scale_linetype_manual(values=linetypev)
) + geom_point(data=vitdf,aes(y=viterbi),
               lwd=2.5,alpha=1,colour="black",shape=118,show.legend=FALSE)
## adding geom_point() with vitdf screws up the legend ...
@
...
(ggplot(acfdf,aes(x=lag,y=ACF,colour=model,
                  lwd=model,alpha=model,linetype=model)) 
  + geom_line()
  + xlab("Lag") 
  ...
  + scale_size_manual(values=lwdvec)
  + scale_linetype_manual(values=linetypev)
) + geom_point(data=vitacf,lwd=2.5,alpha=1,colour="black",shape=118,
             )
...
%% - HMMs are widely used; need to work harder to make them appropriately complex (*)
%% - recapitulate problem: big data overfits/large number of states is problematic (*)
%% - adding temphet goes in the right direction (i.e. sims work, model is better/lower BIC, fewer states, less complex/fewer parameters overall)
%% - GOALS: identifying states and/or association of behaviour with environment
%%   *or* prediction
%% - need for more diagnostics (e.g. Potts et al 2014); identify mismatches
...
\textbf{BMB: I think we want one (or two?) paragraph(s) here that revisit(s)
the deeper statistical issues of 
\begin{enumerate}
\item that different approaches are needed for prediction vs. state identification. If we're
aiming for state identification it might be OK to pre-specify the
number of states and, \emph{post hoc}, match (Viterbi-based) state
estimates with environmental variation (à la Fryxell et al.). 
If we have specific
environmental covariates in mind, though, it might be better to
fit a covariate-dependent model so that we can explicitly model
the switching process \ldots  If we want to do prediction, we have
to include the covariates, because Viterbi can only estimate
state occupancy if observed movements are available \ldots
\item prediction (of movements) vs estimation of number of states also
represent two different questions; \citep{celeux_deviance_2006} (I haven't
read it!) and \href{http://www.modulad.fr/archives/JPFenelon/Prog-JPF-2006/Celeux/CycleFenelon_Conference_Gilles_Celeux.pdf}{Celeux presentation} discuss this;
BIC is good for prediction, but not necessarily for estimation of number
of states.  We don't need to go into detail, but should point in this direction. Also perhaps hint a bit more above where we reference
\citep{dean_behavioural_2013}.
\end{enumerate}
}
...
these can both test overall
goodness-of-fit \citep{potts_generalized_2014} and,
more importantly, localize fitting problems to particular aspects of the
...
\todo{does it make any sense to compress figs~\ref{fig:adj_BIC_comparisons_cat2} etc. into a single plot (via facetgrid or multi-lines)?  Same idea for all repeated plots; also, update to use percent-+-percent syntax}
