...
as independent observations and neglecting possible correlations
...
the six states biologically.
We follow their lead in using BIC-optimality (i.e., minimum
...
of underlying movement states.  Using BIC also simplifies \ldots
\todo{cite S. Richards Ecology paper?  it's easier to see if BIC
identifies the true model than to see if AIC finds the model with
the minimum K-L distance; admit that tapering effect sizes do
happen here; claim similar results with AIC, although
generally higher number of states identified}
...
HMM, we need to sum over all possible hidden sequences. This direct method is computationally inefficient, but there exist more efficient ways to computation the likelihood and
...
$\pi_{ij} = \exp(\eta_{ij}(t))/\left(1+\sum_{j=2}^{n}\exp(\eta_{ij}(t))\right),$ for $j={2,...,n}$
