...
HMMs are widely used and flexible to model these type of movement data. With the increasing volumes and higher resolution data that are available, traditional HMMs will overfit with big data with large number of states that may not be feasible. Models with large numbers of states fit the data well (even when model complexity is penalized) but are hard to interpret biologically, while models with fewer states that are biologically interpretable are not always statistically supported. We need to work harder to make these models appropriate complex that can be both biologically interpretable and statistically defensible. 

We have presented a relatively simple yet little-studied
...
transitions) with simulations and with Florida panther data. HMM with more than three states are not biologically
...
of fit statistically. Our analyses revealed modelling with time-inhomogeneous transitions appear to offer a simple way to (1) reduce the selected number of states to a biologically interpretable level, (2) capture observed diurnal and autocorrelation patterns, (3) provide fitted models that are better (lower BIC)
and, overall, less complex than time-homogeneous models. 

Incorportating temporal heterogenity in animal movement is definitely the right direction, but there are much more work to be done. From the ecological point of view, our model lacks association of behavioural and environmental features that can potentially improve goodness of fit, predictions, and reduce the number of states. It is really hard to optimize and allocate/distribute parameters when increasing model complexity by adding more predictors or adding more states, especially in HMMs where they are state dependent and may have non-linear relation between states and parameters. More diagnostics and tests are needed to identify features, patterns and mismatches in the data and to validate model fitting performance and predictability.       
