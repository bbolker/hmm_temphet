...
step-length distributions\todo{have we run Gamma as well?}, with and without 
a von Mises-distributed turning angle included in the model (Figure~\ref{fig:BICred_plot}).\todo{W/VM/HMM (blue/solid/left panel) should correspond to vdK's results, but that has $\dbic=0$ for 5 states rather than 6. Is that right? Is that HMM/HSMM diff?}
...
Model complexity and number of parameters increase as the number of latent states increase, FMM to HMM, and lastly, FMM and HMM incorporating temporal heterogeneity. 


\todo{Discuss model complexity/parameter-number computations here, or with each model.  Also discuss the ``hourly'' model here.  Make the table into a figure!}
...
We used three approaches to assess the fit of both time-homogeneous and time-inhomogeneous HMMs with 3 to 6 states to step-length data from the four of the thirteen Florida panthers with the most data ($>  $ observations). (1) BIC relative to the optimal-BIC model within each type of transition complexity\todo{BMB: not sure what that means. Clarify?} ($\dbic$ = BIC - min(BIC)). 
(2) Computed average step-length by hour of day for the observed data and for data simulated from the models.
(3) Computed temporal autocorrelation function (ACF) for the observed data and for data simulated from the models. For the second and third metrics (hourly average and ACF), we simulated data from the models because the metrics are hard to compute from first principles for HMMs with temporally varying transition probabilities.
...
<<BICred_plot,warning=FALSE,echo=FALSE,message=FALSE,fig.cap="Change in BIC across models, cat \\#130.  $\\dbic$ is scaled individually per model, so that $\\dbic=0$ represents the BIC-optimal number of states for a particular model.",fig.width=6,fig.height=4>>=
...
As a complement, we also added FMM, FMM with sinusoidal temporal heterogeneities, and HMM with hourly transitions to compare the temporal effects in goodness of fit. Looking at Figure 2, models with temporal heterogenity is better than models without in both FMM and HMM frameworks, but time homogenous HMM models is better than FMM with sinusoidal temporal heterogeneity. Temporal heterogenity is good with good models such as the quadratic and sinuosidal. Multiple block models is good for cats with distinguishable diurnal patterns and hourly model is too complicated. 
...
<<state_plots, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="States">>=
temp5 <- tail(getpars(fitsin5s),10)
temp6 <- tail(getpars(fithomo6s),12)
dat1 <- data.frame(log_mean=temp5[c(1,3,5,7,9)],
                     log_sd=temp5[c(2,4,6,8,10)], Model="HMM + TH(sin)") 
dat2 <- data.frame(log_mean=temp6[c(1,3,5,7,9,11)],
                     log_sd=temp6[c(2,4,6,8,10,12)],Model="HMM") 
dat1 <- dat1[order(dat1$log_mean),]
dat2 <- dat2[order(dat2$log_mean),]
  
dat <- rbind(dat1,dat2)
sd1 <- ggplot(dat,aes(log_mean,log_sd,color=Model))+
    geom_point()+geom_line() + ggtitle("Cat 1")
@

<<state_plots cat2, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="States">>=
load("cat2sumdat.RData")
temp5 <- tail(getpars(fithomo5s),10)
temp4 <- tail(getpars(fitsin4s),8)
dat1 <- data.frame(log_mean=temp5[c(1,3,5,7,9)],
                     log_sd=temp5[c(2,4,6,8,10)], Model="HMM") 
dat2 <- data.frame(log_mean=temp4[c(1,3,5,7)],
                     log_sd=temp4[c(2,4,6,8)],Model="HMM + TH(sin)") 
dat1 <- dat1[order(dat1$log_mean),]
dat2 <- dat2[order(dat2$log_mean),]
  
dat <- rbind(dat2,dat1)
sd2 <- ggplot(dat,aes(log_mean,log_sd,color=Model))+
    geom_point()+geom_line() + ggtitle("Cat 2")
@

<<state_plots cat14, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="States">>=
...
dat <- rbind(dat2,dat1)
sd14 <- ggplot(dat,aes(log_mean,log_sd,color=Model))+
    geom_point()+geom_line() + ggtitle("Cat 14")
@

<<state_plots cat15, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="States">>=
...
dat1 <- data.frame(log_mean=temp5[c(1,3,5,7,9)],
                     log_sd=temp5[c(2,4,6,8,10)], Model="HMM") 
dat2 <- data.frame(log_mean=temp6[c(1,3,5,7,9,11)],
                     log_sd=temp6[c(2,4,6,8,10,12)],Model="HMM + TH(sin)") 
dat1 <- dat1[order(dat1$log_mean),]
dat2 <- dat2[order(dat2$log_mean),]
  
dat <- rbind(dat1,dat2)
sd15 <- ggplot(dat,aes(log_mean,log_sd,color=Model))+
    geom_point()+geom_line() + ggtitle("Cat 15")

grid.arrange(sd1,sd2,sd14,sd15)
@
...
<<BICred_plot cat131,warning=FALSE,echo=FALSE,message=FALSE,fig.cap="Change in BIC across models, cat \\#131.  $\\dbic$ is scaled individually per model, so that $\\dbic=0$ represents the BIC-optimal number of states for a particular model.",fig.width=6,fig.height=4>>=
...
<<BICred_plot cat94,warning=FALSE,echo=FALSE,message=FALSE,fig.cap="Change in BIC across models, cat \\#94.  $\\dbic$ is scaled individually per model, so that $\\dbic=0$ represents the BIC-optimal number of states for a particular model.",fig.width=6,fig.height=4>>=
