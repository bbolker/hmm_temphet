...
Model complexity and the number of parameters increase as the number of latent states increase, FMM to HMM, and lastly, FMM and HMM incorporating temporal heterogeneity. The number of free parameters in an HMM can be generalized by summing up the number of free parameters of the three distinct components. Let $n$ be the number of hidden states and $k_{i}, k_{t}, k_{e}$ be the parameters of the prior distribution, transition function and emission distributions. Then the number of free parameters of an HMM is:


\begin{equation}
\underbrace{k_{i}*(n-1)}_{\text{Initial}} 
{+} \underbrace{k_{t}*n*(n-1)}_{\text{Transition}}
{+} \underbrace{ k_{e}*n}_{\text{Emission}}
\label{eq:numparam}
\end{equation}

When increasing in number of states, the number of free parameters in time-homogeneous FMMs and HMMs and FMMs with temporal heterogenity will increase linearly, whereas, HMMs with temporal heterogenity will not by Eq.~\ref{eq:numparam}. When comparing BICs, it is important to account for the tradeoff between log-likelihood and number of states, but also log-likelihood and number of free parameters.
...
We used this approach to validate our models and comparing these models with the observed movements instead of the standard Viterbi predictions by the Viterbi algorithm because Viterbi predictions \emph{double count} the observed data. The Viterbi algorithm solves/predicts for the most probable sequence of movement states based on the observations \citep{zucchini_hidden_2009,langrock_flexible_2012}. It is useful to predict missing data in the observation sequence, but because it is conditional on the observed values, it can not relibly evaluate goodness of fit for the different structral complexities of HMM models. 
...
We simulated a two-state HMM with sinusoidal temporal transitions 100 times and fitted it with two to five state HMMs and without temporal transition.  Figure~\ref{fig:sim_results} shows heterogeneous transition models can always predict the correct number of states, whereas, can over estiate the number of states via BIC-optimal approach. 
...
complicated models. This approach allows the time homogeneous transition model's prediction to capture the temporal heterogeneity from the observations. The limitation of the Viterbi appoach is that it cannot predict future movements beyond the observation sequence. In general, the time homogeneous
