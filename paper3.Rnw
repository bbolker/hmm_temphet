...
allmodsumdf2 <- readRDS("cat.1.df.RDS")$multimoddf
...
sumdat <- transform(readRDS("cat.1.df.RDS")$HMMsumdf,
                    mod = ifelse(grepl("HMM",model),"no","yes")
)
linetypev <- c(rep(2,2),rep(1,5))
(adj_BIC_plot <- ggplot(sumdat, aes(x=nstates,y=deltaBIC,colour=model,linetype=model))+
