...
simplot <- ggplot(ICdat,aes(x=nstates,y=n,color=model))+
                        ...
                        labs(y="Counts",x="BIC-optimal States")
print(simplot)
...
df2 <- transform(allmodsumdf2, #change allmodsumdf to allmodsumdf2
            ...
            homog=ifelse(grepl("TH",type),"Time-inhomogeneous HMM","Homogeneous HMM"))
...
<<adj_BIC_comparisons, warning=FALSE, echo=FALSE, message=FALSE,fig.cap="Adjusted BIC by number of free parameters for HMM model types. The left panel shows FMM, FMM with a sin prior and HMM. The right panel shows HMMs with different temporal transitions.",fig.width=8>>=
