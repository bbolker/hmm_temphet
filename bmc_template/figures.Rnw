...
<<BICred_plot,warning=FALSE,echo=FALSE,message=FALSE,fig.width=6,fig.height=4>>=
...
<<adj_BIC_comparisons, warning=FALSE, echo=FALSE, message=FALSE,,fig.width=8>>=
...
<<avg_step_length_by_time, message=FALSE, echo=FALSE, warning=FALSE,fig.width=8>>=
...
<<acf_plot, message=FALSE, echo=FALSE, warning=FALSE>>=
acffun <- function(sim){
...
all_cat_pars2 <- ldply(lme4:::namedList(1,22,14,15),get_cat_pars2)

g0 <- ggplot(all_cat_pars2,aes(log_mean,log_sd,color=Model))+
...
allmodsumdf2 <- readRDS("../summary_stats/cat.14.df2.RDS")$multimoddf
df2 <- transform(allmodsumdf2, #change allmodsumdf to allmodsumdf2
            distrib=ifelse(grepl("W",model),"Weibull","log-Normal"),
            turn_angle=ifelse(grepl("VM",as.character(model)),"yes","no"),
            homog=ifelse(grepl("TH",type),"time-geneous HMM","homogeneous HMM"))
...
linetypev <- c(rep(2,2),rep(1,5))
sumdat <- transform(readRDS("../summary_stats/cat.14.df2.RDS")$HMMsumdf,
                     mod = ifelse(type=="HMM + TH","HMM + TH","FMMs and HMM")
)
linetypev <- c(rep(2,2),rep(1,5))
(adj_BIC_plot <- ggplot(sumdat, aes(x=parameters
...
avgdf <- readRDS("../summary_stats/cat.14.df2.RDS")$avgplotdf
