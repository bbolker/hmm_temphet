...
based on the log-likelihood curve rather than the BIC.
However, at least in our simulations the BIC does correctly identify
...
\bmb{mike, can we take a quick try at computing ICL for our data/sims?}

\begin{equation*}
\begin{split}
ICL(m,K) & = \underbrace{\max_{a} \log f(x|\bar z, m, K, a)}_{\textrm{max log-likelihood}} - \frac{\lambda_m,K}{2} \log n \\
 & + \log \Gamma(K/2) + \sum_{k=1}^{K} \log \Gamma(\bar n_k + 1/2) \\
 & - K \log \Gamma(1/2) - \log \Gamma(n + K/2)
\end{split}
\end{equation*}

($K$=number of clusters; $n$=number of obs.; $\lambda_{m,K}$ is ``the
number of free components in $a$'' (i.e., number of free parameters:
p. 8); $\bar n_k$ is, I think, the number of data points which have
their max. probability of being in cluster $k$ [i.e., we should be
  able to get this at least approximately from Viterbi, although I'm
  not sure whether it's more appropriate to use Viterbi or an
  ``instantaneous'' max probability]). The {\tt rebmix} package has an
ICL criterion for its particular objects ({\tt
  sos::findFn("\{integrated classification likelihood\}"}), but I
don't know if it'll be useful.  I think Raftery and Fraley also have a
modified IC for clusters?
